...
const authApiRoutes = async (fastify) => {
    ...
    fastify.post('/forgot-password', async (request, reply) => {
        ...
        if (!body.success)
            return reply.code(400).send({ success: false, message: 'Email tidak valid' });
        const user = await prisma_1.prisma.user.findUnique({ where: { email: body.data.email } });
        // Always return success even if not found to prevent email enumeration attacks
        if (!user)
            return reply.send({ success: true, message: 'Instruksi reset password akan dikirim ke email tersebut.' });
    ...
    fastify.post('/reset-password', async (request, reply) => {
        ...
        const body = schema.safeParse(request.body);
        if (!body.success)
            return reply.code(400).send({ success: false, message: 'Data tidak lengkap' });
        const user = await prisma_1.prisma.user.findUnique({
    ...
    fastify.get('/invite-info', async (request, reply) => {
        ...
        if (!token)
            return reply.sendError('Token wajib disertakan');
        const invite = await prisma_1.prisma.tenantTeams.findUnique({
            where: { token },
            include: { tenant: true }
        });
        if (!invite) {
    ...
    fastify.post('/register', async (request, reply) => {
        const registerSchema = zod_1.z.object({
            ...
            kantorName: zod_1.z.string().optional(),
        ...
        if (token) {
            ...
            return reply.code(201).send({
                ...
                data: {
                    token: jsonwebtoken_1.default.sign({ sub: result.user.id, tenantId: result.tenant.id, role: result.user.role }, JWT_SECRET, { expiresIn: '7d' }),
        ...
        const sessionToken = jsonwebtoken_1.default.sign({ sub: result.user.id, tenantId: result.tenant.id, role: result.user.role }, JWT_SECRET, { expiresIn: '7d' });
                      ...
                      <tr>
                        ...
                        <td style="padding:6px 0;">
                          <span style="background:rgba(99,102,241,0.2);color:#818cf8;font-size:11px;font-weight:700;padding:3px 10px;border-radius:99px;border:1px solid rgba(99,102,241,0.3); text-transform:uppercase;">
                            ${plan ? (planLabel[plan] ?? plan) : 'STARTER'}
                          </span>
                        </td>
                      </tr>
                    </table>
...
exports.authApiRoutes = authApiRoutes;
