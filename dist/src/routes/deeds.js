...
const deedRoutes = async (fastify) => {
    ...
    fastify.get('/', async (request, reply) => {
        ...
        try {
            ...
            const activeDeleteRequests = await prisma_1.prisma.notification.findMany({
                where: {
                    tenantId,
                    type: 'URGENT',
                    isRead: false
            ...
            activeDeleteRequests.forEach((notif) => {
            ...
            const safeDeeds = JSON.parse(JSON.stringify(deeds, (key, value) => typeof value === 'bigint' ? Number(value) : value)).map((deed) => ({
                ...deed,
    ...
    fastify.patch('/:id/attachments/:attachmentId', async (request, reply) => {
        ...
        if (!tenantId)
            return reply.sendError('Tenant ID wajib disertakan');
        if (!name || name.trim() === '')
            return reply.sendError('Nama file wajib diisi');
        try {
            const deed = await prisma_1.prisma.deed.findFirst({ where: { id, tenantId, deletedAt: null } });
            if (!deed)
                return reply.sendError('Akta tidak ditemukan', 404);
            let attachments = [];
            if (deed.attachments) {
    ...
    fastify.post('/:id/request-delete', async (request, reply) => {
        ...
        try {
            ...
            const tenantNotifications = await prisma_1.prisma.notification.findMany({
                where: {
                    tenantId,
                    type: 'URGENT',
                    isRead: false
            ...
            const existingNotif = tenantNotifications.find((notif) => notif.actionUrl && notif.actionUrl.includes(`approveDelete=${id}`));
            if (existingNotif) {
            ...
            const notaries = await prisma_1.prisma.user.findMany({
                where: {
                    tenantId,
                    role: 'NOTARIS',
                    deletedAt: null
            ...
            await Promise.all(notaries.map((notary) => prisma_1.prisma.notification.create({
                data: {
                    tenantId,
                    userId: notary.id,
    ...
    fastify.post('/:id/approve-delete', async (request, reply) => {
        ...
        try {
            ...
            const relatedNotifications = await prisma_1.prisma.notification.findMany({
                where: {
                    tenantId,
                    isRead: false
                }
            ...
            const targetNotifIds = relatedNotifications
                .filter((n) => n.actionUrl && n.actionUrl.includes(`approveDelete=${id}`))
                .map((n) => n.id);
